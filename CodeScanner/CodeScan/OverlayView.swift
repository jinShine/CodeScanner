...
//    focusView = FocusView(frame: CGRect(x: center.x - (size.width / 2) - , y: bounds.midY - (size.height / 2), width: size.width, height: size.height))
    focusView = FocusView(frame: CGRect(x: center.x - (size.width / 2), y: center.y - (size.height / 2), width: size.width, height: size.height))
